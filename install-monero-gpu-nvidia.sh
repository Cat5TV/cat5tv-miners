...
# Build XMRIG
mkdir -p $dest/software && cd $dest/software && git clone https://github.com/xmrig/xmrig-nvidia && mv xmrig-nvidia $name && cd $name
cmake .
cd src
grep -rl --include="*.cpp" "std::isnormal" | xargs sed -i 's/std::isnormal/isnormal/g'
cd ..
make
