...
# This script installs Turtlecoin GPU mining (AMD) on a Debian-based Linux system
...
echo "$dest/software/$name/xmrig-amd --algo=cryptonight-lite --variant=1 -o trtl.pool.mine2gether.com:\$port -u $wallet -p $worker -k --donate-level=1" >> $dest/$name.sh
